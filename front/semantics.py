...
class Semantics:
    def __init__(self, DOMI, tokens):
        self.tokens = tokens
        self.functionsR = ["getchar","for", "scanf","printf"]
        self.DOMI = DOMI
        self.contexts = []
        aux =""
    ...
    def verifyType(self, token, verifyContext):
        if(token["type"].value == self.DOMI.NUMBER.value):
            ...
            if(re.match('[0-9]+.[0-9]+',token["token"])):
                verifyContext["type"] = "float"
            else:
    ...
    def check(self):
        ...
        for i in range(0, len(self.tokens) - 1):
            ...
            if(self.tokens[i]["token"] == "return"):
                ...
                if(self.tokens[i-1]["token"] == "int" or self.tokens[i-1]["token"] == "float" or self.tokens[i-1]["token"] == "void" or self.tokens[i-1]["token"] == "char" or self.tokens[i-1]["token"] =="string"):
                    menssage = "A variavel ("+self.tokens[i]["token"]+") já foi declarada nesse escopo"
                    if(self.tokens[i+1]["token"] == "("):

                        if(not(objAux == False or returned) and objAux["type"] != "void"):
                        ...
                        lastType = self.tokens[i-1]["token"]
                        contAux = 0
                        while(True):
                            atrib = True
                            if(self.tokens[i]["type"].value == self.DOMI.VARIABLE.value):
                                menssage = "A variavel ("+self.tokens[i]["token"]+") já foi declarada nesse escopo"
                                for index in range(0, len(self.contexts[len(self.contexts)-1]["variables"])):
                                    if(self.contexts[len(self.contexts)-1]["variables"][index]['name'] == self.tokens[i]["token"] and self.tokens[i-1]["token"] == ","):
                                        okay = True
                                if(okay):
                                    self.printError(historic,menssage) 
                                    result = False    
                            ...
                            i += 1
                            contAux += 1
                            if(self.tokens[i]["token"] == ";" or (self.tokens[i]["token"] != "," and self.tokens[i]["type"].value != self.DOMI.LITERAL.value and self.tokens[i]["type"].value != self.DOMI.NUMBER.value and self.tokens[i]["type"].value != self.DOMI.VARIABLE.value and self.tokens[i]["type"].value != self.DOMI.ASSIGNMENT.value and self.tokens[i]["type"].value != self.DOMI.OPERATOR.value)):
                                i = i - contAux
                                break
                        historic += " " + self.tokens[i]["token"]
                            
                if(not (self.tokens[i-1]["token"] == "int"or self.tokens[i-1]["token"] == "void" or self.tokens[i-1]["token"] == "float" or self.tokens[i-1]["token"] == "char" or self.tokens[i-1]["token"] =="string") or atrib):
                    atrib = False
                        ...
                        if(self.tokens[i+1]["type"].value == self.DOMI.ASSIGNMENT.value or self.tokens[i+1]["type"].value  == self.DOMI.COMPARATIVE.value or self.tokens[i+1]["type"].value  == self.DOMI.OPERATOR.value):
                            while(True):
                                ...
                                if(verifyContext["type"] != typeT) and (not self.tokens[i+2]["token"] in self.functionsR):
                                    if (typeT == "float" and verifyContext["type"] == "int") or (typeT == "int" and verifyContext["type"] == "float")  or (typeT == "int" and verifyContext["type"] == "float" and self.tokens[i+1]["token"] != "=" or verifyContext["type"] == "any" or self.tokens[i+1]["token"] == "<<"):
                                        okay = True
                                        break
